...
**✅ UNBLOCKED & LIVE 2026-07-21:** the JSI-107 CC authorize is cleared. The preference sync (`ConstantContactSyncQueueable('PREFERENCE')`) ran clean end-to-end as a Queueable (Integration_Log = Success) and is now **scheduled hourly** as part of `JSI CC Inbound Sync Hourly`. First run logged "no memberships to process" (no contact list-membership changes in the last ~25h) — real-data confirmation lands on the first live membership change.

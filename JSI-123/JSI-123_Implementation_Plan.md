...
| D14 | ~~Multiple boards~~ → **member TYPES on ONE board** (CORRECTED 2026-07-29) | **`Board_Member_Type__c` config object** — JCRC Recruited / UJA Appointed / UJA Observer, each w/ base commitment (5k/5k/1.8k) + term length (6/6/1). `Board_Term__c` → **`Member_Type__c`** lookup. **Single board** (D13 restored). Jason corrected the wrong multi-board model. | **Corrected ✓ 2026-07-29** |
| D21 | Board Term auto-generation (Jason 2026-07-29) | On save: **auto-fill End Date** = Start + type term length; **pre-create one Board Membership Year per fiscal year** of the term (idempotent). Roll-off = count of **started** non-officer years ≥ term length (`BoardYearAdvance` schedulable flips years as they begin). Removed dead fields `Annual_Commitment_Default__c` + `Term_Number__c`. | **Locked ✓ 2026-07-29** |
...
**Phase 3 — CORRECTION (Jason feedback 2026-07-29): single board + member types + auto-generation.**
The multi-board model was wrong (see D14 correction) and a new Board Term generated nothing. Fixed:
- **`Board__c` → `Board_Member_Type__c`** (renamed object; re-seeded JCRC Recruited / UJA Appointed / UJA Observer). `Board_Term.Board__c` → **`Member_Type__c`**; dropped `Board_Meeting.Board__c` (single board). Updated `Must_Roll_Off` + `BoardCommitmentService` to read `Member_Type__r`.
- **Removed dead fields** `Board_Term.Annual_Commitment_Default__c` (superseded by member-type base + auto-derive) + `Board_Term.Term_Number__c` (unused) — Jason confirmed.
- **`BoardTermTrigger` + `BoardTermService`:** on save, **auto-fill End Date** = Start + type term length, and **pre-create one Board Membership Year per fiscal year** (idempotent, keyed on term+FY-label).
- **Started-year roll-off:** `Counts_Toward_Limit` now also requires the year to have started (FY_Start ≤ today); **`BoardYearAdvance`** schedulable (scheduled July 1 = CronTrigger "JSI-123 Board Year Advance") flips years as they begin; commitment stamps only started years.
- **Deleted** old `Board__c` object + 3 seed records + 4 fields (org + repo); cleaned repo profiles. **Tests 6/6** (incl. BoardMembersControllerTest); **end-to-end verified**: create JCRC term → End Date 2032-06-30, 6 per-year rows, $5k commitment, Years Served 1. Updated Board Term record page (Member Type + Board Years Served).
- **GOTCHA:** `commit` is a **reserved Apex identifier** (like `map`/`into`) — can't be a variable/param name.
